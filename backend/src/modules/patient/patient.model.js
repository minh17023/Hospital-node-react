...
export const PatientModel = {
  async findByCCCD(soCCCD) {
    const [rows] = await pool.query(
      `SELECT * FROM BenhNhan WHERE soCCCD=? LIMIT 1`, [soCCCD]
    );
    return rows[0] || null;
  },
  ...
  async findById(idBenhNhan) {
    const [rows] = await pool.query(
      `SELECT * FROM BenhNhan WHERE idBenhNhan=? LIMIT 1`, [idBenhNhan]
    );
    return rows[0] || null;
  },
  ...
  async update(idBenhNhan, patch) {
    const allow = [
      ...
      "diaChi","soDienThoai","email","ngheNghiep","tinhTrangHonNhan",
    ...
    const [rs] = await pool.query(
      `UPDATE BenhNhan SET ${fields.join(", ")} WHERE idBenhNhan=?`, values
    );
